            ...
            <div className="space-y-8 fade-in">
               <div className="glass p-10 rounded-[40px] border border-white/5 relative overflow-hidden">
                  ...
                  <div className="relative z-10 flex flex-col md:flex-row items-center gap-8 md:gap-12">
                     <div className="w-24 h-24 md:w-32 md:h-32 rounded-[32px] bg-accent/10 flex items-center justify-center text-accent">
                        <Warehouse size={48} />
                     </div>
                     <div className="flex-1 text-center md:text-left space-y-4">
                        <h2 className="text-3xl md:text-5xl font-black text-white italic uppercase tracking-tighter">Gestão de Pesqueiros</h2>
                        <p className="text-gray-400 max-w-xl">
                          Transforme seu pesqueiro em um ponto oficial e gerencie eventos, torneios e anúncios em tempo real para toda a comunidade WikiFish.
                        </p>
                        
                        {!isResortOwner ? (
                           <button 
                              onClick={() => setShowResortForm(true)}
                              className="btn-primary" 
                              style={{ padding: '12px 24px', borderRadius: 16 }}
                           >
                              <Plus size={20} /> Cadastrar meu Pesqueiro
                           </button>
                        ) : (
                           <div className="flex flex-wrap gap-4">
                              <a href="/resort-admin" className="btn-primary" style={{ padding: '12px 24px', borderRadius: 16 }}>
                                 Ir para Painel Administrativo <ArrowRight size={18} className="ml-2" />
                              </a>
                           </div>
                        )}
                     </div>
               ...
               {/* Benefícios Partner Section */}
               <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {[
                    { title: 'Torneios Oficiais', desc: 'Lance eventos com venda de ingressos integrada.', icon: Trophy },
                    { title: 'Visibilidade Elite', desc: 'Seu pesqueiro aparece como destaque dourado no mapa.', icon: Star },
                    { title: 'Gestão Viva', desc: 'Atualize preços e status de peixes instantaneamente.', icon: Clock },
                  ].map(b => (
                    <div key={b.title} className="glass p-6 rounded-3xl border border-white/5 space-y-3">
