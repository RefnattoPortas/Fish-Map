    ...
    <div className="flex w-screen h-screen overflow-hidden bg-[#0a0f1a]">
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-12">
        <div className="max-w-5xl mx-auto space-y-10 fade-in">
          ...
          {/* Hero Profile Header */}
          <div className="relative glass-elevated rounded-[32px] md:rounded-[40px] p-6 md:p-12 border border-white/5 overflow-hidden">
            <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-accent/5 blur-[120px] rounded-full -mr-32 -mt-32" />
            
            <div className="relative flex flex-row items-start text-left gap-4 md:gap-12">
              
              {/* Left Column (Avatar, Rank, Action Button) */}
              <div className="flex flex-col items-center gap-2 shrink-0 max-w-[80px] md:max-w-none">
                {/* Avatar Big */}
                <div className="relative">
                  <div 
                    className="w-16 h-16 md:w-44 md:h-44 rounded-2xl md:rounded-[48px] overflow-hidden p-0.5 md:p-1.5 shadow-2xl"
                    style={{ background: `linear-gradient(135deg, ${userRank.color}, transparent)` }}
                  >
                    <div className="w-full h-full rounded-[14px] md:rounded-[42px] overflow-hidden bg-slate-900 border border-white/10">
                      {profile.avatar_url ? (
                        <img src={profile.avatar_url} className="w-full h-full object-cover" />
                      ) : <User className="w-full h-full p-4 md:p-10 text-gray-500" />}
                    </div>
                  </div>
                </div>

                {/* Rank (Patente) */}
                <div className="inline-flex items-center gap-1 px-2 md:px-4 py-1 md:py-1.5 bg-white/5 border border-white/10 rounded-full mt-1">
                  <userRank.icon className="w-3 h-3 md:w-4 md:h-4 shrink-0" style={{ color: userRank.color }} />
                  <span className="text-[7px] md:text-xs font-black uppercase tracking-[0.2em] md:tracking-[0.3em] overflow-hidden text-ellipsis whitespace-nowrap" style={{ color: userRank.color }}>
                    {userRank.title}
                  </span>
                </div>
                ...
                {currentUser && currentUser.id !== profile.id && (
                  <button 
                    ...
                    className={`px-2 md:px-5 py-1.5 md:py-2 rounded-lg md:rounded-xl border font-black uppercase tracking-widest text-[7px] md:text-[10px] transition-all flex flex-col md:flex-row items-center justify-center gap-1 shadow-lg mt-1 w-full text-center leading-tight ${
                      isFollowing 
                  ...
                  >
                    {isFollowing ? <UserMinus size={10} className="md:w-4 md:h-4" /> : <UserPlus size={10} className="md:w-4 md:h-4" />}
                    <span>{isFollowing ? 'Remover' : 'Adicionar'}</span>
                  </button>
                )}
              </div>

              {/* Profile Info (Name, Location, Bio) */}
              <div className="flex-1 space-y-1.5 md:space-y-4 pt-1 md:pt-0 overflow-hidden">
                
                <h1 className="text-xl md:text-6xl font-black text-white uppercase tracking-tighter leading-none italic truncate">
                  {profile.display_name || profile.username || 'Pescador'}
                </h1>
                
                <p className="text-accent font-bold text-[9px] md:text-xs flex items-center justify-start gap-1 truncate">
                  <MapPin size={10} className="shrink-0 md:w-3 md:h-3" /> <span className="truncate">{profile.city || '---'}, {profile.state || '---'}</span>
                </p>
                
                <p className="text-gray-400 font-medium text-[10px] md:text-base max-w-xl leading-snug">
          ...
          {/* Stats Mobile List / Desktop Grid */}
          <div className="flex flex-col md:grid md:grid-cols-4 gap-2 md:gap-6">
            {[
              ...
              <div key={stat.label} className="glass p-3 md:p-6 rounded-xl md:rounded-[28px] border border-white/5 flex flex-row md:flex-col items-center md:items-start justify-between md:justify-start hover:bg-white/[0.05] transition-all group">
                
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 md:w-10 md:h-10 rounded-[10px] md:rounded-xl flex items-center justify-center" style={{ backgroundColor: `${stat.color}15`, color: stat.color }}>
                    <stat.icon className="w-4 h-4 md:w-5 md:h-5" />
                  </div>
                  <p className="md:hidden text-[9px] font-black text-gray-400 uppercase tracking-widest">{stat.label}</p>
                </div>

                <div className="text-right md:text-left md:mt-3">
                  <p className="text-lg md:text-3xl font-black text-white">{stat.value}</p>
                  <p className="hidden md:block text-[9px] font-black text-gray-500 uppercase tracking-widest">{stat.label}</p>
                </div>
